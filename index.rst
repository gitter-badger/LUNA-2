.. Luna Research Tool documentation master file, created by
   Luna-quickstart on Sun Aug  5 08:36:52 2018.
   You can adapt this file completely to your liking, but it should at least
   contain the root `toctree` directive.

Welcome to the Luna Research Tool's documentation!
==============================================
...
Install Luna, either from a distribution package or from
`PyPI <https://pypi.python.org/pypi/Luna>`_ with ::
...
The root directory of a Luna collection of reStructuredText document sources
is called the :term:`source directory`.  This directory also contains the Luna
configuration file :file:`conf.py`, where you can configure all aspects of how
Luna reads your sources and builds your documentation.  [#]_

Luna comes with a script called :program:`Luna-quickstart` that sets up a
...
:program:`Luna-apidoc`; see :doc:`/man/Luna-apidoc` for details.


Defining document structure
---------------------------

Let's assume you've run :program:`Luna-quickstart`.  It created a source
...
(or *toctree*).  This is one of the main things that Luna adds to
...
where the toctree directive is placed.  Also, Luna now knows about the order
...
In Luna source files, you can use most features of standard reStructuredText.
There are also several features added by Luna.  For example, you can add
...
reStructuredText and :ref:`Lunamarkup` for a full list of markup added by
Luna.
...
docs.  A build is started with the :program:`Luna-build` program, called like
this::

   $ Luna-build -b html sourcedir builddir

where *sourcedir* is the :term:`source directory`, and *builddir* is the
directory in which you want to place the built documentation.
The :option:`-b <Luna-build -b>` option selects a builder; in this example
Luna will build HTML files.

|more| Refer to the :program:`Luna-build man page <Luna-build>` for all
options that :program:`Luna-build` supports.

However, :program:`Luna-quickstart` script creates a :file:`Makefile` and a
...
.. admonition:: How do I generate PDF documents?
   ...
   <Luna.builders.latex.LaTeXBuilder>` and readily invokes the pdfTeX
   toolchain for you.
...
One of Luna's main objectives is easy documentation of :dfn:`objects` (in a
...
Luna will find it and create a link to it.
...
Earlier we mentioned that the :file:`conf.py` file controls how Luna processes
...
Luna, you can do non-trivial tasks in it, like extending :data:`sys.path` or
importing a module to find out the version you are documenting.

The config values that you probably want to change are already put into the
:file:`conf.py` by :program:`Luna-quickstart` and initially commented out
...
config value that is not automatically added by :program:`Luna-quickstart`,
...
source files, in documentation strings.  Luna supports the inclusion of
docstrings from your modules with an :dfn:`extension` (an extension is a Python
module that provides additional features for Luna projects) called "autodoc".

In order to use autodoc, you need to activate it in :file:`conf.py` by putting
the string ``'Luna.ext.autodoc'`` into the list assigned to the
...
.. warning::
   ...
   :mod:`~Luna.ext.autodoc` **imports** the modules to be documented.  If any
   modules have side effects on import, these will be executed by ``autodoc``
   when ``Luna-build`` is run.

   If you document scripts (as opposed to library modules), make sure their main
   routine is protected by a ``if __name__ == '__main__'`` condition.

|more| See :mod:`Luna.ext.autodoc` for the complete description of the
features of autodoc.

InterLuna
-----------

Many Luna documents including the `Python documentation`_ are published on the
internet.  When you want to make links to such documents from your
documentation, you can do it with :mod:`Luna.ext.interLuna`.

.. _Python documentation: https://docs.python.org/3

In order to use interLuna, you need to activate it in :file:`conf.py` by
putting the string ``'Luna.ext.interLuna'`` into the :confval:`extensions`
list and set up the :confval:`interLuna_mapping` config value.

For example, to link to ``io.open()`` in the Python library manual, you need to
setup your :confval:`interLuna_mapping` like::

   interLuna_mapping = {'python': ('https://docs.python.org/3', None)}
...
:confval:`interLuna_mapping` (this needs access to the URL in order to
download the list of valid targets).  InterLuna also works for some other
:ref:`domains' <domains>` roles including ``:ref:``, however it doesn't work for
``:doc:`` as that is non-domain role.

|more| See :mod:`Luna.ext.interLuna` for the complete description of the
features of interLuna.
...
.. [#] This is the usual layout.  However, :file:`conf.py` can also live in
       ...
       :program:`Luna-build man page <Luna-build>` for more information.
